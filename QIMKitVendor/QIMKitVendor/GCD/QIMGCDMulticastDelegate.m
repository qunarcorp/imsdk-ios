...
@interface QIMGCDMulticastDelegateNode : NSObject {
@private
...
@interface QIMGCDMulticastDelegate () {
...
@interface QIMGCDMulticastDelegateEnumerator () {
    NSUInteger numNodes;
...
@implementation QIMGCDMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue {
    ...
    QIMGCDMulticastDelegateNode *node =
            [[QIMGCDMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue {
    ...
    for (i = [delegateNodes count]; i > 0; i--) {
        QIMGCDMulticastDelegateNode *node = [delegateNodes objectAtIndex:(i - 1)];
        ...
        if (delegate == nodeDelegate) {
            if ((delegateQueue == NULL) || (delegateQueue == node.delegateQueue)) {
                // Recall that this node may be retained by a QIMGCDMulticastDelegateEnumerator.
...
- (void)removeAllDelegates {
    for (QIMGCDMulticastDelegateNode *node in delegateNodes) {
        node.delegate = nil;
#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
        node.unsafeDelegate = nil;
#endif
...
- (QIMGCDMulticastDelegateEnumerator *)delegateEnumerator {
    return [[QIMGCDMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation QIMGCDMulticastDelegateNode
...
@implementation QIMGCDMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr {
    while (currentNodeIndex < numNodes) {
        QIMGCDMulticastDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
        currentNodeIndex++;
