...
@property (nonatomic, strong) UIWebView *schemeView;
...
- (UIWebView *)schemeView {
    if (!_schemeView) {
        _schemeView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, _contentView.width, _contentView.height)];
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"open_scheme" ofType:@"html"];
        NSString *htmlString = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
        [_schemeView loadHTMLString:htmlString baseURL:nil];
    }
    return _schemeView;
}
...
- (void)customTabBar:(QIMCustomTabBar *)tabBar didSelectIndex:(NSUInteger)index {
    ...
    } else if ([tabBarId isEqualToString:[NSBundle qim_localizedStringForKey:@"tab_title_scheme"]]) {
        [_contentView addSubview:self.schemeView];
        [_schemeView setHidden:NO];
    } else {
