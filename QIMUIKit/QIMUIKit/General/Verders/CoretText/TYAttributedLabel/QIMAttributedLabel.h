//
//  QIMAttributedLabel.h
//  QIMAttributedLabelDemo
//
//  Created by chenjie on 16/7/7.
//  Copyright (c) 2016年 chenjie. All rights reserved.
//  QIMAttributedLabel v2.0 verson

#import "QIMCommonUIFramework.h"
#import "QIMTextContainer.h"

typedef NS_ENUM(NSUInteger, QCVerticalAlignment) {
    QCVerticalAlignmentTop,
    QCVerticalAlignmentCenter,
    QCVerticalAlignmentBottom,
};

@class QIMAttributedLabel;
@protocol QIMAttributedLabelDelegate <NSObject>
@optional
...
/**
 *  QIMAttributedLabel 属性文本 支持图文混排显示，支持添加image和UIView，支持自定义排版
 */
@interface QIMAttributedLabel : UIView

@property (nonatomic, assign) id<QIMAttributedLabelDelegate>     delegate;
...
@property (nonatomic, assign)   QCVerticalAlignment verticalAlignment; // 垂直对齐方式 默认是向上对齐
 ...
 *
 *  @param textRunArray textRun数组（需遵循协议QIMTextStorageProtocol,否则不会添加）
 */
...
@interface QIMAttributedLabel (AppendAttributedString)
/**
 *  追加(添加到最后) 普通文本
 *
 *  @param text 普通文本
 ...
 *
 *  @param textStorageArray 自定义run数组（需遵循协议QCAppendTextStorageProtocol,否则不会添加）
 */
...
@interface QIMAttributedLabel (Link)
...
@interface QIMAttributedLabel (UIImage)
...
- (void)addImage:(UIImage *)image range:(NSRange)range
                                   size:(CGSize)size
                              alignment: (QCDrawAlignment)alignment;
