...
NSString *const kQCTextRunAttributedName = @"QCTextRunAttributedName";
...
- (BOOL)enumerateRunRectContainPoint:(CGPoint)point
                        ...
                        successBlock:(void (^)(id<QIMTextStorageProtocol> textStorage))successBlock;

- (BOOL)enumerateLinkRectContainPoint:(CGPoint)point
                           viewHeight:(CGFloat)viewHeight
                         successBlock:(void (^)(id<QCLinkStorageProtocol> linkStorage))successBlock;

@end

@interface QIMAttributedLabel ()<UIGestureRecognizerDelegate>
{
    struct {
...
@implementation QIMAttributedLabel
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        [self setupProperty];
        _textContainer = [[QIMTextContainer alloc]init];
    }
    return self;
}
...
- (instancetype)initWithTextContainer:(QIMTextContainer *)textContainer
{
    if (self = [super init]) {
        [self setupProperty];
...
- (void)setTextContainer:(QIMTextContainer *)attStringCreater
{
...
- (void)setDelegate:(id<QIMAttributedLabelDelegate>)delegate
{
...
- (void)clearOwnerView {
    for (id storage in self.textContainer.textStorages) {
        if ([storage respondsToSelector:@selector(setOwnerView:)]) {
            [(id<QIMViewStorageProtocol>)storage setOwnerView:nil];
        }
    }
}
...
{
    // draw storage
    [_textContainer enumerateDrawRectDictionaryUsingBlock:^(id<QIMDrawStorageProtocol> drawStorage, CGRect rect) {
        if ([drawStorage conformsToProtocol:@protocol(QIMViewStorageProtocol) ]) {
            [(id<QIMViewStorageProtocol>)drawStorage setOwnerView:self];
        }
...
- (void)singleTap:(UITapGestureRecognizer *)sender
{
    ...
    [_textContainer enumerateRunRectContainPoint:point viewHeight:CGRectGetHeight(self.frame) successBlock:^(id<QIMTextStorageProtocol> textStorage){
        if (_delegateFlags.textStorageClickedAtPoint) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([_textContainer existLinkRectDictionary]) {
        ...
        __typeof (self) __weak weakSelf = self;
        [_textContainer enumerateLinkRectContainPoint:point viewHeight:CGRectGetHeight(self.frame) successBlock:^(id<QCLinkStorageProtocol> linkStorage) {
...
@implementation QIMAttributedLabel (AppendAttributedString)
...
@implementation QIMAttributedLabel (Link)
...
@implementation QIMAttributedLabel (UIImage)
...
- (void)addImageWithName:(NSString *)imageName range:(NSRange)range size:(CGSize)size alignment:(QCDrawAlignment)alignment
{
...
{
    [self addImageWithName:imageName range:range size:size alignment:QCDrawAlignmentTop];
}
...
- (void)appendImageWithName:(NSString *)imageName size:(CGSize)size alignment:(QCDrawAlignment)alignment
{
...
{
    [self appendImageWithName:imageName size:size alignment:QCDrawAlignmentTop];
}
...
@implementation QIMAttributedLabel (UIView)
