...
static void * QIMMWVideoPlayerObservation = &QIMMWVideoPlayerObservation;

@interface QIMMWPhotoBrowser () <LCActionSheetDelegate>

@end

@implementation QIMMWPhotoBrowser
...
- (void)_initialisation {
    ...
    // Listen for QIMMWPhoto notifications
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleQIMMWPhotoLoadingDidEndNotification:)
                                                 name:QIMMWPHOTO_LOADING_DID_END_NOTIFICATION
                                               object:nil];
...
- (void)viewDidLoad {
    ...
    if (self.displayNavArrows) {
        NSString *arrowPathFormat = @"QIMMWPhotoBrowser.bundle/UIBarButtonItemArrow%@";
    ...
    _indexLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.frame) - 50, CGRectGetWidth(self.view.frame), 30)];
    [_indexLabel setAutoresizingMask:UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleTopMargin|UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin];
    _indexLabel.textAlignment = NSTextAlignmentCenter;
    _indexLabel.textColor = [UIColor whiteColor];
    _indexLabel.font = [UIFont systemFontOfSize:14];
    _indexLabel.backgroundColor = [UIColor clearColor];
    [self.view addSubview:_indexLabel];
    if (_notAutoHidenControls) {
...
- (void)performLayout {
    ...
    if (_enableGrid) {
        hasItems = YES;
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"QIMMWPhotoBrowser.bundle/UIBarButtonItemGrid" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)]];
    } else {
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_viewHasAppearedInitially) {
        if (_autoPlayOnAppear) {
            QIMMWPhoto *photo = [self photoAtIndex:_currentPageIndex];
...
- (void)willMoveToParentViewController:(UIViewController *)parent {
    if (parent && _hasBelongedToViewController) {
        [NSException raise:@"QIMMWPhotoBrowser Instance Reuse" format:@"QIMMWPhotoBrowser instances cannot be reused."];
    }
}
...
- (void)layoutVisiblePages {
	...
	for (QIMMWZoomingScrollView *page in _visiblePages) {
...
- (id<QIMMWPhoto>)photoAtIndex:(NSUInteger)index {
    id <QIMMWPhoto> photo = nil;
...
- (id<QIMMWPhoto>)thumbPhotoAtIndex:(NSUInteger)index {
    id <QIMMWPhoto> photo = nil;
...
- (QIMMWCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    QIMMWCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <QIMMWPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[QIMMWCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (void)loadAdjacentPhotosIfNecessary:(id<QIMMWPhoto>)photo {
    QIMMWZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <QIMMWPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
                    QIMMWLog(@"Pre-loading image at index %lu", (unsigned long)pageIndex-1);
                }
            }
...
#pragma mark - QIMMWPhoto Loading Notification

- (void)handleQIMMWPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <QIMMWPhoto> photo = [notification object];
    QIMMWZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImage]) {
            // Successful load
...
- (void)tilePages {
	...
	for (QIMMWZoomingScrollView *page in _visiblePages) {
		...
		if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
			...
			QIMMWLog(@"Removed page at index %lu", (unsigned long)pageIndex);
		}
	}
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
			QIMMWZoomingScrollView *page = [self dequeueRecycledPage];
			if (!page) {
				page = [[QIMMWZoomingScrollView alloc] initWithPhotoBrowser:self];
			}
			[_visiblePages addObject:page];
			[self configurePage:page forIndex:index];

			[_pagingScrollView addSubview:page];
			QIMMWLog(@"Added page at index %lu", (unsigned long)index);
            
            // Add caption
            QIMMWCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
            ...
            if (page.displayingVideo) {
                ...
                [playButton setImage:[UIImage imageForResourcePath:@"QIMMWPhotoBrowser.bundle/PlayButtonOverlayLarge" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateNormal];
                [playButton setImage:[UIImage imageForResourcePath:@"QIMMWPhotoBrowser.bundle/PlayButtonOverlayLargeTap" ofType:@"png" inBundle:[NSBundle bundleForClass:[self class]]] forState:UIControlStateHighlighted];
...
- (void)updateVisiblePageStates {
    ...
    for (QIMMWZoomingScrollView *page in copy) {
...
- (QIMMWZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
	QIMMWZoomingScrollView *thePage = nil;
	for (QIMMWZoomingScrollView *page in _visiblePages) {
		if (page.index == index) {
			thePage = page; break;
		}
...
- (QIMMWZoomingScrollView *)pageDisplayingPhoto:(id<QIMMWPhoto>)photo {
	QIMMWZoomingScrollView *thePage = nil;
	for (QIMMWZoomingScrollView *page in _visiblePages) {
		if (page.photo == photo) {
			thePage = page; break;
...
- (void)configurePage:(QIMMWZoomingScrollView *)page forIndex:(NSUInteger)index {
	page.frame = [self frameForPageAtIndex:index];
    page.index = index;
    page.photo = [self photoAtIndex:index];
}

- (QIMMWZoomingScrollView *)dequeueRecycledPage {
	QIMMWZoomingScrollView *page = [_recycledPages anyObject];
	if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) { 
            ...
            if (photo != [NSNull null]) {
                ...
                QIMMWLog(@"Released underlying image at index %lu", (unsigned long)i);
            }
        }
    }
    ...
    id <QIMMWPhoto> currentPhoto = [self photoAtIndex:index];
...
- (CGRect)frameForCaptionView:(QIMMWCaptionView *)captionView atIndex:(NSUInteger)index {
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController = [[QIMMWGridViewController alloc] init];
...
- (void)hideGrid {
    ...
    QIMMWGridViewController *tmpGridController = _gridController;
    _gridController = nil;
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    // Status bar
    if (!_leaveStatusBarAlone && self.notAutoHidenControls == NO) {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        for (QIMMWZoomingScrollView *page in _visiblePages) {
            if (page.captionView) {
                QIMMWCaptionView *v = page.captionView;
...
- (void)actionButtonPressed:(id)sender {
    ...
    id <QIMMWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            if (![[QIMMWQRCodeActivity sharedInstance] canPerformQRCodeWithImage:[photo underlyingImage]]) {
...
- (void)actionSheet:(LCActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    switch (buttonIndex) {
        ...
        case 4: {
            if ([[QIMMWQRCodeActivity sharedInstance] canPerformQRCodeWithImage:[photo underlyingImage]]) {
                [[QIMMWQRCodeActivity sharedInstance] setFromPhotoBrowser:self];
                [[QIMMWQRCodeActivity sharedInstance] performActivity];
            } else {
                id <QIMMWPhoto> photo = [self photoAtIndex:_currentPageIndex];
...
- (void)saveImageWithPhotoData:(NSData *)photoData {
    if (photoData.length > 0) {
