...
+ (UIViewController *)getVCWithNavigation:(UINavigationController *)navVC
    ...
    rootView.frame = CGRectMake(0, [[QIMDeviceManager sharedInstance] getSTATUS_BAR_HEIGHT] - 20, vc.view.width, vc.view.height);
...
//        UINavigationController *navVC = (UINavigationController *)[[[UIApplication sharedApplication] keyWindow] rootViewController];
        ...
        [chatVC setName:@"QIMlucas"];
        [chatVC setTitle:@"QIMlucas"];
        [chatVC setChatType:ChatType_SingleChat];
        [[NSNotificationCenter defaultCenter] postNotificationName:kNotifySelectTab object:@(0)];
        [navVC popToRootVCThenPush:chatVC animated:YES];
    });
...
//本地消息搜索
RCT_EXPORT_METHOD(searchLocalMessageByKeyword:(NSString *)keyword :(NSString *)xmppid :(NSString *)realjid:(RCTResponseSenderBlock)callback){
    NSMutableArray *message = [[QIMKit sharedInstance] searchLocalMessageByKeyword:keyword XmppId:xmppid RealJid:realjid];
    NSMutableArray *result = [NSMutableArray arrayWithCapacity:3];
    for (NSDictionary *msg in message) {
        NSMutableDictionary *tempMsg = [[NSMutableDictionary alloc] initWithDictionary:msg];
        NSString *msgFrom = [msg objectForKey:@"from"];
        NSString *headerUrl = [[QIMImageManager sharedInstance] qim_getHeaderCachePathWithJid:msgFrom];
        [tempMsg setQIMSafeObject:headerUrl forKey:@"headerUrl"];
        [result addObject:tempMsg];
    }
