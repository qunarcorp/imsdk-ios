...
typedef enum {
    ...
    QIMXMenuViewArrowDirectionNone,
    QIMXMenuViewArrowDirectionUp,
    QIMXMenuViewArrowDirectionDown,
    QIMXMenuViewArrowDirectionLeft,
    QIMXMenuViewArrowDirectionRight,
    
} QIMXMenuViewArrowDirection;


@interface QIMXMenuView : UIView
...
@interface QIMXMenuOverlay : UIView
...
#pragma mark - QIMXMenuOverlay

@implementation QIMXMenuOverlay
...
- (void)singleTap:(UITapGestureRecognizer *)recognizer {
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[QIMXMenuView class]] && [view respondsToSelector:@selector(dismissMenu:)]) {
...
#pragma mark - QIMXMenuView
@implementation QIMXMenuView {
    QIMXMenuViewArrowDirection _arrowDirection;
    CGFloat _arrowPosition;
    UIView *_contentView;
    NSArray *_menuItems;
    QIMXMenuSelectedItem _selectedItem;
}
...
- (id)init {
    ...
    if(self) {
        ...
        if ([QIMXMenu hasShadow]) {
            self.layer.shadowOpacity = 0.5;
            self.layer.shadowColor = [QIMXMenu tintColor].CGColor;
...
- (void)setupFrameInView:(UIView *)view fromRect:(CGRect)fromRect {
    ...
    if (heightPlusArrow < (outerHeight - rectY1)) {
        ...
        _arrowDirection = QIMXMenuViewArrowDirectionUp;
        ...
        _arrowDirection = QIMXMenuViewArrowDirectionDown;
        CGPoint point = (CGPoint){
            rectXM - widthHalf,
        ...
        _arrowDirection = QIMXMenuViewArrowDirectionLeft;
        CGPoint point = (CGPoint){
            rectX1,
            rectYM - heightHalf
        };
        ...
        _arrowDirection = QIMXMenuViewArrowDirectionRight;
        CGPoint point = (CGPoint){
        ...
        _arrowDirection = QIMXMenuViewArrowDirectionNone;
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(QIMXMenuSelectedItem)selectedItem {
    ...
    QIMXMenuOverlay *overlay = [[QIMXMenuOverlay alloc] initWithFrame:view.bounds];
    ...
    if (self.superview) {
        if (animated) {
            ...
            [UIView animateWithDuration:0.2 animations:^(void) {
                ...
                if ([self.superview isKindOfClass:[QIMXMenuOverlay class]])
...
- (void)performAction:(id)sender {
    ...
    QIMXMenuItem *menuItem = _menuItems[button.tag];
...
- (UIView *) mkContentView {
    ...
    UIFont *titleFont = [QIMXMenu titleFont];
    
    CGFloat maxImageWidth = 0;
    CGFloat maxItemHeight = 0;
    CGFloat maxItemWidth = 0;
    
    for (QIMXMenuItem *menuItem in _menuItems) {
    ...
    UIImage *selectedImage = [QIMXMenuView selectedImage:(CGSize){maxItemWidth, maxItemHeight + 2}];
    UIImage *gradientLine = [QIMXMenuView gradientLine: (CGSize){maxItemWidth - kMarginX * 4, 1}];
...
- (CGPoint)arrowPoint {
    ...
    if (_arrowDirection == QIMXMenuViewArrowDirectionUp) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMinY(self.frame) };
        
    } else if (_arrowDirection == QIMXMenuViewArrowDirectionDown) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame) + _arrowPosition, CGRectGetMaxY(self.frame) };
        
    } else if (_arrowDirection == QIMXMenuViewArrowDirectionLeft) {
        
        point = (CGPoint){ CGRectGetMinX(self.frame), CGRectGetMinY(self.frame) + _arrowPosition  };
        
    } else if (_arrowDirection == QIMXMenuViewArrowDirectionRight) {
...
- (void)drawBackground:(CGRect)frame inContext:(CGContextRef) context {
    ...
    UIColor *tintColor = [QIMXMenu tintColor];
    if (tintColor) {
        CGFloat a;
    ...
    if ([QIMXMenu backgrounColorEffect] == QIMXMenuBackgrounColorEffectGradient) {
        R1-=0.2;
        G1-=0.2;
        B1-=0.2;
    }
    ...
    if (_arrowDirection == QIMXMenuViewArrowDirectionLeft ||
        _arrowDirection == QIMXMenuViewArrowDirectionRight) {
...
#pragma mark - QIMXMenu
/// MenuView
static QIMXMenu                      *gMenu;
/// 背景色
static UIColor                      *gTintColor;
/// 字体
static UIFont                       *gTitleFont;
/// 背景色效果
static QIMXMenuBackgrounColorEffect   gBackgroundColorEffect = QIMXMenuBackgrounColorEffectSolid;
/// 是否显示阴影
static BOOL                          gHasShadow = NO;

@implementation QIMXMenu {
    QIMXMenuView *_menuView;
    BOOL         _observing;
}
...
+ (void)reset {
    gTintColor = nil;
    gTitleFont = nil;
    gBackgroundColorEffect = QIMXMenuBackgrounColorEffectSolid;
    gHasShadow = NO;
}
...
- (void)showMenuInView:(UIView *)view fromRect:(CGRect)rect menuItems:(NSArray *)menuItems selected:(QIMXMenuSelectedItem)selectedItem {
    ...
    // 创建MenuView
    _menuView = [[QIMXMenuView alloc] init];
