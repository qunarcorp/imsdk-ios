...
@interface QIMChatToolBar ()<QTalkTextViewDelegate>
...
//    self.recordBtn = [[QIMRecordButton alloc] init];
    ...
    self.textView.accessibilityIdentifier = @"ChatTextView";
    
    [self addSubview:self.voiceBtn];
    [self.voiceBtn setAccessibilityIdentifier:@"voiceBtn"];
    [self addSubview:self.faceBtn];
    [self.faceBtn setAccessibilityIdentifier:@"faceBtn"];
    [self addSubview:self.moreBtn];
    [self.moreBtn setAccessibilityIdentifier:@"moreBtn"];
...
//    [self addSubview:self.recordBtn];
    ...
    self.recordBtn.recordTouchUpInsideAction = ^(QIMRecordButton *sender){
        QIMVerboseLog(@"完成录音");
    ...
    self.recordBtn.recordTouchUpOutsideAction = ^(QIMRecordButton *sender){
        QIMVerboseLog(@"取消录音");
        [sender setButtonStateWithNormal];
    ...
    //持续调用
    self.recordBtn.recordTouchDragInsideAction = ^(QIMRecordButton *sender){
    };
    //持续调用
    self.recordBtn.recordTouchDragOutsideAction = ^(QIMRecordButton *sender){
    };
    
    //中间状态  从 TouchDragInside ---> TouchDragOutside
    self.recordBtn.recordTouchDragExitAction = ^(QIMRecordButton *sender){
        QIMVerboseLog(@"将要取消录音");
    ...
    self.recordBtn.recordTouchDragEnterAction = ^(QIMRecordButton *sender){
        QIMVerboseLog(@"继续录音");
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if ([text isEqualToString:@"\n"])
    {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(chatToolBarTextView:shouldChangeTextInRange:replacementText:)]) {
            if ([text isEqualToString:@"@"]) {
//                if (self.chatToolTextViewShow == NO) {
                    [self.delegate chatToolBarTextView:self.textView shouldChangeTextInRange:range replacementText:text];
//                }
            } else {
                [self.delegate chatToolBarTextView:self.textView shouldChangeTextInRange:range replacementText:text];
            }
...
- (CGFloat)getTextViewContentH:(QTalkTextView *)textView {
...
- (void)layoutAndAnimateTextView:(QTalkTextView *)textView
{
    ...
    if (changeInHeight != 0.0f && fabs(changeInHeight) >= 10) {
