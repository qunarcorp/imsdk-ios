...
/* 整个bar的宽度 */
#define kQIMVideoPlayProgressViewWidth (self.frame.size.width - (kBtnWith*0.5)*2)
/* slider 的高度 */
#define  kPlayVideoProgressBarHeight 3

@interface QTalkVideoProgressSliderBtn : UIButton
 ...
 *  为了让拖动按钮变得更大
 */
@implementation QTalkVideoProgressSliderBtn{
...
@implementation QIMVideoPlayProgressView{
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat showY = (self.frame.size.height - kPlayVideoProgressBarHeight)*0.5;
        
        /* 背景 */
        _bgProgressView = [[UIView alloc] initWithFrame:CGRectMake(kBtnWith*0.5, showY, kQIMVideoPlayProgressViewWidth, kPlayVideoProgressBarHeight)];
        _bgProgressView.backgroundColor = [UIColor blackColor];
        [self addSubview:_bgProgressView];
        
        /* 缓存进度 */
        _ableBufferProgressView = [[UIView alloc] initWithFrame:CGRectMake(kBtnWith*0.5, showY, 0, kPlayVideoProgressBarHeight)];
        _ableBufferProgressView.backgroundColor = [UIColor yellowColor];
        [self addSubview:_ableBufferProgressView];
        
        /* 播放进度 */
        _finishPlayProgressView = [[UIView alloc] initWithFrame:CGRectMake(kBtnWith*0.5, showY, 0, kPlayVideoProgressBarHeight)];
        _finishPlayProgressView.backgroundColor = [UIColor redColor];
        [self addSubview:_finishPlayProgressView];
        
        /* 滑动按钮 */
        _sliderBtn = [[QTalkVideoProgressSliderBtn alloc] initWithFrame:CGRectMake(0, showY, 44, 44)];
