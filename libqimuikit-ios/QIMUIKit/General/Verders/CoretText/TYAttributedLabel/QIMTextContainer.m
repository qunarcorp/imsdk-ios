...
@implementation QIMTextContainer
...
- (void)addTextStoragesWithAtrributedString:(NSMutableAttributedString *)attString
{
    if (attString && _textStorageArray.count > 0) {
        ...
        for (id<QIMTextStorageProtocol> textStorage in _textStorageArray) {
            
            // 修正图片替换字符来的误差
            if ([textStorage conformsToProtocol:@protocol(QIMDrawStorageProtocol) ]) {
                continue;
            }
            
            if ([textStorage conformsToProtocol:@protocol(QCLinkStorageProtocol)]) {
                if (!((id<QCLinkStorageProtocol>)textStorage).textColor) {
                    ((id<QCLinkStorageProtocol>)textStorage).textColor = _linkColor;
                }
            }
        ...
        for (id<QIMTextStorageProtocol> textStorage in _textStorageArray) {
            ...
            if ([textStorage conformsToProtocol:@protocol(QIMDrawStorageProtocol)]) {
                id<QIMDrawStorageProtocol> drawStorage = (id<QIMDrawStorageProtocol>)textStorage;
...
{
    [textStorageArray sortUsingComparator:^NSComparisonResult(id<QIMTextStorageProtocol> obj1, id<QIMTextStorageProtocol> obj2) {
...
- (void)saveTextStorageRectWithFrame:(CTFrameRef)frame
{
    ...
    for (int i = 0; i < numberOfLines; i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            id<QIMTextStorageProtocol> textStorage = [attributes objectForKey:kQCTextRunAttributedName];
            ...
            if (textStorage) {
                ...
                } else if ([textStorage conformsToProtocol:@protocol(QCLinkStorageProtocol)]) {
...
- (void)addRunRectDictionary:(NSDictionary *)runRectDictionary
{
    if (runRectDictionary.count < _runRectDictionary.count) {
        ...
        // 遍历不会画出来的
        for (id<QIMTextStorageProtocol>drawStorage in drawStorageArray) {
            if ([drawStorage conformsToProtocol:@protocol(QIMViewStorageProtocol)]) {
                [(id<QIMViewStorageProtocol>)drawStorage didNotDrawRun];
            }
        }
    }
...
- (void)enumerateDrawRectDictionaryUsingBlock:(void (^)(id<QIMDrawStorageProtocol> drawStorage, CGRect rect))block
{
    [_drawRectDictionary enumerateKeysAndObjectsUsingBlock:^(NSValue *rectValue, id<QIMDrawStorageProtocol> drawStorage, BOOL * stop) {
        if (block) {
...
- (BOOL)enumerateRunRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<QIMTextStorageProtocol> textStorage))successBlock
{
...
- (BOOL)enumerateRunRect:(NSDictionary *)runRectDic ContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(id<QIMTextStorageProtocol> textStorage))successBlock
{
    ...
    // 遍历run位置字典
    [runRectDic enumerateKeysAndObjectsUsingBlock:^(NSValue *keyRectValue, id<QIMTextStorageProtocol> textStorage, BOOL *stop) {
        
        CGRect imgRect = [keyRectValue CGRectValue];
        CGRect rect = CGRectApplyAffineTransform(imgRect, transform);
        
        if ([textStorage conformsToProtocol:@protocol(QIMDrawStorageProtocol) ]) {
            rect = UIEdgeInsetsInsetRect(rect,((id<QIMDrawStorageProtocol>)textStorage).margin);
        }
...
@implementation QIMTextContainer (Add)

- (void)addTextStorage:(id<QIMTextStorageProtocol>)textStorage
{
    if (textStorage) {
...
{
    if (textStorageArray) {
        for (id<QIMTextStorageProtocol> textStorage in textStorageArray) {
            if ([textStorage conformsToProtocol:@protocol(QIMTextStorageProtocol)]) {
...
@implementation QIMTextContainer (Append)
...
- (void)appendTextStorage:(id<QCAppendTextStorageProtocol>)textStorage
{
    if (textStorage) {
        if ([textStorage conformsToProtocol:@protocol(QIMDrawStorageProtocol)]) {
            [(id<QIMDrawStorageProtocol>)textStorage setTextfontAscent:_font.ascender descent:_font.descender];
        } else if ([textStorage conformsToProtocol:@protocol(QCLinkStorageProtocol)]) {
            if (!((id<QCLinkStorageProtocol>)textStorage).textColor) {
                ((id<QCLinkStorageProtocol>)textStorage).textColor = _linkColor;
            }
...
{
    if (textStorageArray) {
        for (id<QCAppendTextStorageProtocol> textStorage in textStorageArray) {
            if ([textStorage conformsToProtocol:@protocol(QCAppendTextStorageProtocol)]) {
                [self appendTextStorage:textStorage];
            }
        }
    }
    
    if (![self textStorages]) {
        _textStorages = [[NSMutableArray alloc] initWithArray:textStorageArray];
    } else {
        [_textStorages arrayByAddingObjectsFromArray:textStorageArray];
    }
}
