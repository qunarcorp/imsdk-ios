...
+ (QIMMWPhoto *)photoWithURL:(NSURL *)url {
    return [[QIMMWPhoto alloc] initWithURL:url];
}

+ (QIMMWPhoto *)photoWithAsset:(PHAsset *)asset targetSize:(CGSize)targetSize {
    return [[QIMMWPhoto alloc] initWithAsset:asset targetSize:targetSize];
}

+ (QIMMWPhoto *)videoWithURL:(NSURL *)url {
    return [[QIMMWPhoto alloc] initWithVideoURL:url];
}
            ...
            @try {
                ...
                [assetslibrary assetForURL:url
                              ...
                              failureBlock:^(NSError *error) {
                                  self.underlyingImage = nil;
                                  QIMMWLog(@"Photo from asset library error: %@",error);
                                  [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                              }];
            } @catch (NSException *e) {
                QIMMWLog(@"Photo from asset library error: %@", e);
                [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
            }
        }
    });
