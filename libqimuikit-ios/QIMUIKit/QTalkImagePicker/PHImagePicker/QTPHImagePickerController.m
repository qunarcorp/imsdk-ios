...
//                                    @(PHAssetCollectionSubtypeSmartAlbumPanoramas)];
        _customSmartCollections = @[@(PHAssetCollectionSubtypeAlbumRegular),
        ...
        // UI Customisation
        _pickerBackgroundColor = [UIColor spectralColorBlueColor];
        ...
        _pickerFontHeaderSize = 14.0f;
        
        _navigationBarBackgroundColor = [UIColor spectralColorBlueColor];
        _navigationBarTextColor = [UIColor whiteColor];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (_navigationController.toolbar.subviews.count) {
...
//    [self updateDoneButton];
    ...
    if (!self.allowsMultipleSelection) {
        if (self.confirmSingleSelection) {
            NSString *message = self.confirmSingleSelectionPrompt ? self.confirmSingleSelectionPrompt : [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"picker.confirm.message",  @"GMImagePicker", [NSBundle bundleForClass:QTPHImagePickerController.class],  @"Do you want to select the image you tapped on?")];
            
            [[[UIAlertView alloc] initWithTitle:[NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"picker.confirm.title",  @"GMImagePicker", [NSBundle bundleForClass:QTPHImagePickerController.class],  @"Are You Sure?")]
                                        message:message
                                       delegate:self
                              cancelButtonTitle:[NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"picker.action.no",  @"GMImagePicker", [NSBundle bundleForClass:QTPHImagePickerController.class],  @"No")]
                              otherButtonTitles:[NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"picker.action.yes",  @"GMImagePicker", [NSBundle bundleForClass:QTPHImagePickerController.class],  @"Yes")], nil] show];
        } else {
            [self finishPickingAssets:self];
        }
...
- (void)finishEditWithImage:(UIImage *)image{
    if ([self.delegate respondsToSelector:@selector(assetsPickerController:didFinishEditWithImage:)]) {
        [self.delegate assetsPickerController:self didFinishEditWithImage:image];
    }
}
...
- (void)cameraButtonPressed:(UIBarButtonItem *)button
{
    ...
    if (self.autoSelectCameraImages && [self.navigationController.topViewController isKindOfClass:[QTPHAlbumsViewController class]]) {
        [((QTPHAlbumsViewController *)self.navigationController.topViewController) selectAllAlbumsCell];
    }
